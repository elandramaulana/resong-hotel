...
class InhouseController extends Controller
{
    ...
    {
        $Data = [
            'Title' => "Daftar History Checkout"
        ];
        return view('frontoffice.guest.his_checkout', $Data);
    }
    public function his_checkout(Request $request)
    {
        ## Read value
        ...
        // Total records
        $records = Checkin::join('checkouts', 'checkins.id', '=', 'checkouts.checkin_id')
        ...
        $records = Checkin::orderBy($columnName, $columnSortOrder)
            ->join('checkouts', 'checkins.id', '=', 'checkouts.checkin_id')
            ->join('rooms', 'rooms.id', '=', 'checkins.room_id')
            ->join('guests', 'guests.id', '=', 'checkins.guest_id')
        ...
        foreach ($listData as $item_data) {
            ...
            $data_arr[] = array(
                ...
                "time_checkin" => $item_data->time_checkin,
                "date_checkout" => $item_data->date_checkout,
                "time_checkout" => $item_data->time_checkout,
    ...
    {
        $getExtraBed = LatePointSetting::first();
        ...
        $detailData = [
            ...
            'item_name' => "Extrabed",
            'item_price' => $getExtraBed->extrabed_price,
            'item_qty' => 1,

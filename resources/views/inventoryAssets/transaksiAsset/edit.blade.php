    ...
    <section id="">
        ...
        <div class="container-fluid">
            ...
            <div class="d-sm-flex align-items-center justify-content-start">
                <h1 class="h3 mb-0 text-gray-800">Edit Transaksi Asset</h1>
            </div>
        </div>
            ...
            <div class="container-fluid mt-4">
                <div class="card">
                    <div class="card-body text-dark">
                        ...
                        @if ($trans->trans_jenis == 'MASUK')
                            <form action="{{ route('inventory-assets.trans.update-masuk', $trans->id) }}" method="post"
                                ...
                                <div class="row">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input readonly value="{{ $trans->trans_jenis ?? '' }}" name="trans_jenis"
                                            type="text" class="form-control" id="trans_jenis">
                                        ...
                                        <select name="asset_id" class="form-control" id="asset_id">
                                            @foreach ($assets as $asset)
                                                <option value="{{ $asset->id }}"
                                                    {{ $asset->id == $trans->asset_id ? 'selected' : '' }}>
                                                    {{ $asset->nama }}</option>
                                            @endforeach
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input value="{{ $trans->trans_jml ?? '' }}" name="trans_jml" type="number"
                                            class="form-control" id="trans_jml">
                                        <x-input-error :messages="$errors->get('trans_jml')" class="mt-2" />
                                    </div>

                                    <div class="mb-3">
                                        <label for="Channel" class="form-label">Supplier</label>
                                        <select name="supplier_asset_id" class="form-control" id="supplier_asset_id">
                                            @foreach ($suppliers as $supplier)
                                                <option value="{{ $supplier->id }}"
                                                    {{ $supplier->id == $trans->supplier_asset_id ? 'selected' : '' }}>
                                                    {{ $supplier->supplier_name }}</option>
                                            @endforeach

                                        </select>
                                        <x-input-error :messages="$errors->get('supplier_asset_id')" class="mt-2" />
                                    </div>

                                    <div class="mb-3">
                                        <label for="trans_harga" class="form-label">Total Biaya (Rp)</label>
                                        <input value="{{ $trans->trans_harga ?? '' }}" name="trans_harga" type="number"
                                            class="form-control" id="trans_harga">
                        ...
                        @else
                            <form action="{{ route('inventory-assets.trans.update-keluar', $trans->id) }}" method="post"
                                enctype="multipart/form-data">
                                @csrf
                                <div class="row">
                                    <div class="mb-3">
                                        ...
                                        <select name="trans_jenis" class="form-control" id="trans_jenis">
                                            <option value="BAIK" {{ $trans->trans_jenis == 'BAIK' ? 'selected' : '' }}>
                                                BAIK</option>
                                            <option value="RUSAK" {{ $trans->trans_jenis == 'RUSAK' ? 'selected' : '' }}>
                                                RUSAK
                                            </option>
                                        </select>
                                        <x-input-error :messages="$errors->get('trans_jenis')" class="mt-2" />
